...
public class BonusPurchaseMenu extends InventoryGUI
{
    ...
    public BonusPurchaseMenu(ItemStack sellingItem, List<Integer> pricesAnix, List<Integer> pricesBonus, List<String> benefit) {
        super(false);
                    ...
                    .consumer(event -> {
                        if (event.getClick().isRightClick() && event.getSlot() == sellingItemSlots.get(sellingItemSlots.size() - 1)) {
                            ...
                            if (!sufficientMoney) {
                                ...
                                playerData.getBoughtBonuses().put(Utils.stripColor(this.sellingItem.getItemMeta().getDisplayName()), period);

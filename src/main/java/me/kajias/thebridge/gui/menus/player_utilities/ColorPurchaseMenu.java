...
public class ColorPurchaseMenu extends InventoryGUI
{
    ...
    public ColorPurchaseMenu(ItemStack sellingItem, List<Integer> pricesAnix, List<Integer> pricesBonus, List<String> benefit) {
        super(false);
                    ...
                    .consumer(event -> {
                        if (event.getClick().isRightClick() && event.getSlot() == sellingItemSlots.get(sellingItemSlots.size() - 1)) {
                            ...
                            if (!sufficientMoney) {
                                ...
                                String color = String.valueOf(sellingItem.getDurability());
                                gamePlayer.getBoughtBlockColors().put(color, period);
                                gamePlayer.setSelectedBlockColor(color);
                                Utils.sendMessage(player, config.getString("messages.color-purchase-successful"));

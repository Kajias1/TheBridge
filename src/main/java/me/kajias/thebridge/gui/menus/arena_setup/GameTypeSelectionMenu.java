...
public class GameTypeSelectionMenu extends InventoryGUI
{
   ...
   public void decorate(Player player) {
      ...
      for (GameType gameType : GameType.values()) {
         ItemStack gameTypeItem = createItem(
                 Material.valueOf(menusConfig.getString("menus.arena-type-selection.arena-type-button.material")),
                 Utils.colorize(menusConfig.getString("menus.arena-type-selection.arena-type-button.name").replace("%type%", gameType.toString())),
                 (short) n
         );
         InventoryButton setTypeButton = new InventoryButton()
                 .creator(player1 -> gameTypeItem)
                 .consumer(event -> {
